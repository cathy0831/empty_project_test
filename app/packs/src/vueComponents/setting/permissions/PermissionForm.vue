...
const checkAllAction = (permKey) => {
  ...
  Object.keys(moduleActionObj).forEach((actionKey) => {
    moduleActionObj[actionKey] = isAllChecked ? null : '1'
  })
}

const controlAction = ({ checked, value }, permKey, actionKey) => {
  ...
  if (actionKey === 'search') {
    if (newValue !== '1') {
      // 取消 search 時，其他功能一併被取消
      Object.keys(current).forEach((currentActionKey) => (current[currentActionKey] = null))
    }
  } else {
    // 非 search 被勾選時，search 被勾選
    if (newValue === '1') current.search = '1'
  }
}
        ...
        <div class="flex flex-wrap gap-4">
          <div v-for="actionKey in permItem.actions" :key="`${permItem.module}_${actionKey}`">
            <BaseCheckbox
              ...
              @update:modelValue="controlAction($event, permItem.module, actionKey)"
            />
          </div>
        </div>
      </div>
